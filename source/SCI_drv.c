...
interrupt void SCI_rx_interrupt(void)
{
    ...
    while(SCI_MODUL.SCIFFRX.bit.RXFFST != 0)
    {
        ...
        //and put in queue
        queue_empty = SCI_queue_put(data);

        // ce je vrsta polna ali ce je prislo do preliva

        if (   (queue_empty == FALSE)
            || (SCI_MODUL.SCIFFRX.bit.RXFFOVF == 1))
        {
            // povecam stevec overflow-ov
            ...
            // ni vec nobenega paketa
            SCI_rx_packets = 0;
            // grem ven iz sprejemanja
            break;
        }
...
/**************************************************************
* funkcija ki prejme n bytov po serijskem vodilu
...
* returns:  TRUE, èe je elemnt uspešno vstavljen
...
int SCI_get_packet(int *dst, int buff_size)
{
    ...
    if (SCI_rx_packets != 0)
    {
        // da poberem cel paket - zna biti problem pri dolgih paketih
        SCI_disable_rx_interrupts();
        ...
        do
        {
            ...
            // ce je paket prevelik, potem grem ven
            if (index >= buff_size)
            {
                asm(" ESTOP0");
                //break;
